...
import { convertCompaniesTimeSlots, filterCompaniesByType, handleFetchCompaniesDataError, sortTimeSlots } from "services/apis/companies/companiesApiHelper";
...
describe('Companies API helper', () => {
    ...
    it('handleFetchCompaniesDataError: should return ErrorCustom object', () => {
        const consoleError = jest.spyOn(console, 'error').mockImplementation(() => {error: jest.fn()});
