...
function ReservationsTable () {
    ...
    const getCompanies = async () => {
        setIsLoading(true);
        const result: Company[] | ErrorCustom =
      await CompaniesApiProvider.fetchCompaniesData();
        setIsLoading(false);
    ...
    const companiesList = companiesData?.map((company: Company): JSX.Element => {
        return <CompanyTimeSlots {...company} key={company.id} />;
    });
    ...
    return (
        <div className="container">
            {isLoading
                ...
                : error
                    ? error.message
                    : companiesList && <div className={styles.list}>{companiesList}</div>}
        </div>
    );
}
